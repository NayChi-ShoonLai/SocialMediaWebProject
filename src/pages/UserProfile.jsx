...
import { getSavedPosts } from "../firebaseConfig"; // Import saved posts function
...
import { storage } from "../firebaseConfig"; // ✅ Ensure storage is imported

const UserProfile = ({ handleFollowUpdate, followingList }) => {
      ...
      const updatedPosts = await Promise.all(
        filteredPosts.map(async (post) => {
            ...
            const updatedPhotoUrls = await Promise.all(
                post.photoUrls?.map(async (url, index) => {
                    try {
                        ...
                        return {
                            url: downloadUrl,
                            filter: post.photoFilters?.[index] || { filter: "", brightness: 100, contrast: 100 }
                        };
                    } catch (error) {
            ...
            const updatedVideoUrls = await Promise.all(
                post.videoUrls?.map(async (url, index) => {
                    try {
                        const downloadUrl = await getDownloadURL(ref(storage, url));
                        return {
                            url: downloadUrl,
                            filter: post.videoFilters?.[index] || { filter: "", brightness: 100, contrast: 100 }
                        };
                    } catch (error) {
                        console.error("Error fetching video URL:", error);
                        return {
                            url: "/default-video.jpg",
                            filter: post.videoFilters?.[index] || { filter: "", brightness: 100, contrast: 100 }
                        };
                    }
                }) || []
...
const handleLike = async (postId) => {
    ...
    const fetchUserPosts = async () => {
        ...
        // 🔹 Filter posts based on updated followers list
        const filteredPosts = allPosts.filter(post => {
            if (post.user.uid !== userId) return false; // 🔥 Only show this user's posts
            if (post.visibility === "public") return true; // 🌍 Public posts are always visible
    
            // 🔥 Followers-only posts should appear immediately after following
            return post.visibility === "followers" && followersList.some(follower => follower.id === auth.currentUser?.uid);
        });
...
<div className="mt-6">
  ...
  {userPosts.length === 0 ? (
    ...
    <div className="bg-white border border-gray-200 shadow-xl rounded-3xl p-6 sm:p-8 mb-6 transition-all transform hover:scale-[1.02] hover:shadow-2xl max-w-3xl mx-auto">
            ...
            {post.photoUrls?.map((photo, idx) => (
                <SwiperSlide key={idx}>
                    <img
                        ...
                        style={{
                            filter: getFilterStyle(photo.filter) // ✅ Apply Filter
                        }}
                    />
                </SwiperSlide>
            ))}
            {post.videoUrls?.map((video, idx) => (
                <SwiperSlide key={idx}>
                    <video
                        ...
                        className="w-full h-100 object-cover rounded-lg"
                        style={{
                            filter: getFilterStyle(video.filter) // ✅ Apply Filter
                        }}
                    >
...
>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        <div className="bg-white p-6 rounded-lg shadow-lg w-96 md:w-[600px]">
            ...
            <div 
                ...
                onClick={() => {
                    if (user.id === currentUser?.uid) {
                        navigate(`/profile`); // ✅ Go to logged-in user's profile
                    } else {
                        navigate(`/profile/${user.id}`); // ✅ Go to selected user's profile
                    }
